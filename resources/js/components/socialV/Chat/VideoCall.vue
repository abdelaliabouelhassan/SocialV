    ...
    <div class="header-for-bg">
      <div class="background-header position-relative">
        <img
          ...
          class="img-fluid w-100 rounded rounded"
          alt="profile-bg"
        />
          ...
          <div class="data-block">
            ...
            >
              <div v-if="!callInProgress">
                ...
                <button
                  @click="Call"
                  class="btn btn-outline-success rounded-pill mb-3"
                >
                  Yes Call
                </button>
              </div>
              <div v-if="callInProgress">
                ...
                <button
                  @click="CancelCall"
                  class="btn btn-outline-danger rounded-pill mb-3"
                >
                  Cancel Call
                </button>
              </div>
            ...
            <div
              v-if="
                igotCall == true && showCallVideo == false && IsCallEnd == true
              "
            >
              <div v-if="callInProgress">
            ...
            <div v-show="showCallVideo">
              <h1 class="text-center">
                ...
                {{ igotCall == true ? whoIsCalling.name : user.name }}
              </h1>
              ...
              <button
                @click="StreamStart(displayMediaOptions)"
                class="btn btn-outline-info rounded-pill mb-3"
              >
                Share Screen
              </button>
...
export default {
  ...
  methods: {
    //share screen
    dumpOptionsInfo() {
      let videoElem = this.$refs.stream_vedio;
    ...
    async StreamStart(displayMediaOptions) {
      ...
      try {
        ...
        setTimeout(function () {
          channel.whisper("watchScreen", {
            start: true,
          });
        }, 1000);
         vm.startVideoChat(id);
      } catch (err) {
    ...
    EndCall() {
      ...
      var type = "callEnd";
      this.axios
        .post("/api/SendMessage", { message: message, to: to, type: type })
        .then((response) => {})
        .catch((error) => {
          console.log(error);
        });
    ...
    listen() {
      ...
      Echo.private("VedioCallRuning." + this.$store.state.user.id)
        ...
        .listenForWhisper("NoCancelCall", (e) => {
         ...
         .listenForWhisper("watchScreen", (e) => {
                alert('watch')
        })
    ...
    async setupVideoChat() {
      ...
      this.channel.bind(
        `client-signal-${this.$store.state.user.id}`,
        (signal) => {
